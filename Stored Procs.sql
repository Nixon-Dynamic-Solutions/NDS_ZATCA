...
ELSEIF :Tax = 'KOCO' OR :Tax = '02' THEN
...
CREATE PROCEDURE "@EINVOICE_DETAIL"
(
    IN DocEntry NVARCHAR(50)
)
LANGUAGE SQLSCRIPT
AS
BEGIN
...
(
    SELECT
        ...
        CASE
            WHEN B."DiscPrcnt" > 0
            THEN B."PriceBefDi" * (B."DiscPrcnt" / 100)
            ELSE 0
        END AS "Discount",

        B."LineTotal",

        ROUND(B."LineTotal" * (C."Rate" / 100), 2) AS "vat",
...
-------------------------------------------------------------------
CREATE PROCEDURE "@EINVOICE_HEADER"
(
    IN DocEntry NVARCHAR(50)
)
LANGUAGE SQLSCRIPT
...
SELECT
    ...
    SUBSTRING(A."CreateTS",1,2) || ':' ||
    SUBSTRING(A."CreateTS",3,2) || ':' ||
    SUBSTRING(A."CreateTS",5,2)  AS "CreateTime",
...
SELECT  
    ...
    a."CardName",
    a."DocDate",
    SUBSTRING(a."CreateTS",1,2) || ':' || SUBSTRING(a."CreateTS",3,2) || ':' || SUBSTRING(a."CreateTS",5,2) AS "CreateTime",
    'SAR' AS "DocCur",
    c."U_District",
    d."StreetB",
    d."CityB",
...
BEGIN
    ...
    SELECT
        ...
        'PCE' AS "Unitmsr",
        CASE
            WHEN B."DiscPrcnt" > 0 THEN B."PriceBefDi" * (B."DiscPrcnt" / 100)
            ELSE 0
        END AS "Discount",
